...
import io.github.piyushdaiya.vellumsync.note.SupernoteTotalPathStructuralReport
...
import io.github.piyushdaiya.vellumsync.note.SupernoteVisualPayloadProbe
...
fun NoteInspectorScreen(
    ...
    Column(
        ...
        report.value?.let { inspection ->
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                Button(
                    contentPadding = PaddingValues(horizontal = 20.dp, vertical = 12.dp),
                    onClick = {
                        pendingExportJson.value = inspection.totalPathStructuralReport.singlePageJson(1)
                        exportLauncher.launch("vellumsync-totalpath-structural-${inspection.sha256.take(12)}-page1.json")
                    }
                ) {
                    Text(text = "Export TOTALPATH structural JSON")
                }

                Button(
                    contentPadding = PaddingValues(horizontal = 20.dp, vertical = 12.dp),
                    onClick = {
                        val bytes = if (inspection.cachedCopyPath != null) {
                            ImportedNoteCache.readCachedNote(inspection.cachedCopyPath)
                        } else {
                            null
                        }
                        if (bytes == null) {
                            exportError.value = "No cached read-only copy is available for visual payload probing. Re-import the note first."
                        } else {
                            pendingExportJson.value = SupernoteVisualPayloadProbe.probe(
                                bytes = bytes,
                                report = inspection
                            ).toJson()
                            exportLauncher.launch("vellumsync-visual-payload-probe-${inspection.sha256.take(12)}.json")
                        }
                    }
                ) {
                    Text(text = "Export visual payload probe JSON")
                }
            }
        }
        ...
        report.value?.let { inspection ->
            ...
            TotalPathStructuralReportCard(report = inspection.totalPathStructuralReport)
...
@Composable
private fun TotalPathStructuralReportCard(report: SupernoteTotalPathStructuralReport) {
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(
            modifier = Modifier.padding(16.dp),
            verticalArrangement = Arrangement.spacedBy(6.dp)
        ) {
            Text(text = "TOTALPATH structural parser")
            Text(text = "Format=${report.format}")
            Text(text = "Model=${report.parserModel}")
            Text(text = "Selected page=${report.selectedPageNumber ?: "all"}")
            Text(text = "Pages=${report.pageReports.size}")
            val firstPage = report.pageReports.firstOrNull()
            if (firstPage != null) {
                Text(text = "Page ${firstPage.pageNumber}: ${firstPage.strokeCount} stroke record(s)")
                Text(text = "Screen=${firstPage.screenWidth} x ${firstPage.screenHeight}")
                Text(text = "Boundary model=${firstPage.boundaryModelStatus}")
                Text(text = "Geometric filtering applied=${firstPage.geometricFilteringApplied}")
                firstPage.strokes.take(5).forEach { stroke ->
                    Text(
                        text = "• stroke ${stroke.strokeIndex} record=${stroke.recordIndex} points=${stroke.decodedPointCount} flag=${stroke.flagDrawState} layer=${stroke.strokeLayer ?: "unknown"}"
                    )
                }
            }
            if (report.warnings.isNotEmpty()) {
